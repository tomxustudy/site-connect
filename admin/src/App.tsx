...
export default function AdminApp() {
  ...
  const handleDeleteUser = async (user: UserProfile) => {
    if (!window.confirm(`确定要删除用户 "${user.name}" 吗？此操作不可撤销。`)) return;
    try {
      setLoading(true);
      await axios.delete(`/api/users/${user.id}`, { headers: getHeaders() });
      fetchUsers();
    } catch (err) {
      alert('删除失败');
    } finally {
  ...
  const handleExportExcel = async () => {
    try {
      setLoading(true);
      const res = await axios.get('/api/export/xlsx', {
        headers: getHeaders(),
        responseType: 'blob'
      });
      ...
      link.href = url;
      link.setAttribute('download', `工地数据导出_${new Date().toISOString().slice(0, 10)}.xlsx`);
  ...
  const handleExportPhotos = async () => {
    try {
      setLoading(true);
      const res = await axios.get('/api/export/photos', {
        headers: getHeaders(),
        responseType: 'blob'
      });
      const url = window.URL.createObjectURL(new Blob([res.data]));
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', `现场照片打包_${new Date().toISOString().slice(0, 10)}.zip`);
      document.body.appendChild(link);
      link.click();
      link.remove();
    } catch (err) {
        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          {activeTab === 'export' && (
            <div className="flex-1 overflow-y-auto px-10 py-8 relative">
              <div className="flex justify-between items-center mb-8">
                <div className="flex items-center gap-4">
                  <button onClick={handleExportExcel} className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-lg font-bold flex items-center gap-2 shadow-lg shadow-blue-500/20 text-sm transition-all hover:-translate-y-0.5">
                    <Download size={18} /> 导出 Excel 数据
                  </button>
                  <button onClick={handleExportPhotos} className="bg-green-600 hover:bg-green-700 text-white px-6 py-3 rounded-lg font-bold flex items-center gap-2 shadow-lg shadow-green-500/20 text-sm transition-all hover:-translate-y-0.5">
                    <Box size={18} /> 打包下载现场照片
                  </button>
          ...
          {activeTab === 'settings' && (
            <main className="flex-1 flex flex-col min-w-0 bg-[#F8FAFC]">
                        ...
                        {users.map((u: UserProfile) => (
                          <tr key={u.id} className="hover:bg-slate-50/50 transition-colors group">
                            ...
                            <td className="px-8 py-5">
                              <span className={`px-2 py-1 rounded-full text-[10px] font-black uppercase ${u.role === 'super_admin' ? 'bg-purple-50 text-purple-600' : u.role === 'clerk' ? 'bg-blue-50 text-blue-600' : 'bg-slate-50 text-slate-600'}`}>
                                {u.role === 'super_admin' ? '超级管理员' : u.role === 'clerk' ? '客户文员' : '现场人员'}
                              </span>
                            </td>
                            ...
                            <td className="px-8 py-5 text-right">
                              <div className="flex justify-end gap-2">
                                <button onClick={() => handleEditUser(u)} className="text-slate-300 hover:text-blue-600 transition-colors"><Edit size={16} /></button>
                                <button onClick={() => handleDeleteUser(u)} className="text-slate-300 hover:text-red-600 transition-colors"><Trash2 size={16} /></button>
                              </div>
                            </td>
                          </tr>
                        ))}
              ...
              {/* Modals */}
              {showUserModal && (
                <Modal title={editingUser ? "修改账号" : "新增账号"} onClose={() => { setShowUserModal(false); setEditingUser(null); }}>
                  <UserForm
                    tenants={tenants}
                    isSuperAdmin={currentUser?.role === 'super_admin'}
                    editingUser={editingUser}
                    onSuccess={() => { fetchUsers(); setShowUserModal(false); setEditingUser(null); }}
              ...
              {showPasswordModal && (
                <Modal
                  ...
                  onClose={() => {
                    if (currentUser?.needs_password_change) {
                      alert('为了您的账号安全，首次登录必须修改密码');
                      return;
                    }
                    setShowPasswordModal(false);
                  }}
                  hideCloseButton={!!currentUser?.needs_password_change}
                >
                  <ChangePasswordForm
                    onSuccess={() => {
                      if (currentUser) {
                        const updated = { ...currentUser, needs_password_change: false };
                        setCurrentUser(updated);
                        localStorage.setItem('admin_user', JSON.stringify(updated));
                      }
                      setShowPasswordModal(false);
                      alert('密码修改成功，请记好您的新密码');
                    }}
                    getHeaders={getHeaders}
                  />
                </Modal>
...
function Modal({ title, children, onClose, hideCloseButton }: { title: string, children: React.ReactNode, onClose: () => void, hideCloseButton?: boolean }) {
  return (
...
function UserForm({ tenants, isSuperAdmin, editingUser, onSuccess, getHeaders }: { tenants: Tenant[], isSuperAdmin: boolean, editingUser?: UserProfile | null, onSuccess: () => void, getHeaders: () => any }) {
  const [formData, setFormData] = useState({
    username: '',
    password: editingUser ? '' : '8888',
    name: '',
    phone: '',
    role: 'worker',
    tenant_id_target: ''
  ...
  useEffect(() => {
    if (editingUser) {
      setFormData({
        ...
        tenant_id_target: editingUser.tenant_id?.toString() || ''
      });
    }
  }, [editingUser]);

  const isWorker = formData.role === 'worker';
  const isClerk = formData.role === 'clerk';
  const isSA = formData.role === 'super_admin';
  const needsTenant = isWorker || isClerk;

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (needsTenant && !formData.tenant_id_target) return setError('请选择所属客户');
    if (isWorker && !formData.phone) return setError('现场人员必须填写手机号');
    if (!editingUser && (isClerk || isSA) && !formData.password) return setError('管理员/文员必须设置初始密码');

    setLoading(true);
    setError('');
    try {
      if (editingUser) {
        await axios.put(`/api/users/${editingUser.id}`, formData, { headers: getHeaders() });
      } else {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="space-y-1">
          ...
          <input
            ...
            className="w-full bg-slate-50 border-none rounded-2xl px-5 py-3 text-sm font-bold focus:ring-2 focus:ring-blue-500/20 disabled:opacity-50 transition-all font-mono"
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="space-y-1">
          ...
          >
            ...
            <option value="clerk">客户文员</option>
      ...
      {(isClerk || isSA) && (
        <div className="space-y-1">
          <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-1">
            {editingUser ? '重置密码 (留空则不修改)' : '初始密码 (已预设 8888) *'}
          </label>
          <input
            required={!editingUser}
            type="text"
            placeholder={editingUser ? "••••••••" : "初始密码 8888"}
            className="w-full bg-slate-50 border-none rounded-2xl px-5 py-3 text-sm font-bold focus:ring-2 focus:ring-blue-500/20 transition-all font-mono"
      ...
      >
        {loading ? '提交中...' : (editingUser ? '保存修改' : '确认创建')}
      </button>
    </form>
  );
}
